...
function encodeStringValue(value) {
    ...
    var encoded = fillRightTo64(stringToBuffer(value).toString('hex'));
...
function splitValues(values) {
	...
	for (var k = 0; k < values.length; k += 64)
...
function decodeSplittedStringValue(index, splits) {
	var offset = Math.floor(parseInt('0x' + splits[index], 16) / 32);
	var length = parseInt('0x' + splits[offset], 16);
	
	var nsplits = Math.ceil(length / 32);
	
	var data = splits.slice(offset + 1, offset + 1 + nsplits).join('').substring(0, length * 2);
	
	return stringToBuffer(data, 'hex').toString();
}

function decodeValuesWithTypes(str, types) {
	var value = normalizeHexaString(str);

	var splits = splitValues(value);
